...
impl CollectorController{
    ...
    pub fn route(pool: sqlx::PgPool)->Result<by_axum::axum::Router>{
        ...
        .route("/:id", get(Self::get_collector_by_id).post(Self::act_collector_by_id),
    )
    ...
    pub async fn get_collector(
        ...
        match param{
            CollectorParam::Query(param)=> Ok(Json(CollectorGetResponse::Query(ctrl.query(auth, param).await?)))
        }
